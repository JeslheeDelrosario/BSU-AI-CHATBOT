            ...
            <div className="backdrop-blur-2xl bg-white/5 border border-white/10 rounded-3xl p-8 shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold bg-gradient-to-r from-yellow-300 to-orange-300 bg-clip-text text-transparent flex items-center gap-2">
                  ...
                  {accessibilitySettings.language === 'fil' ? 'Aking mga Badge' : 'My Badges'}
                </h2>
                <Link to="/achievements" className="flex items-center gap-1 text-sm text-cyan-400 hover:text-cyan-300 transition-colors">
                  {accessibilitySettings.language === 'fil' ? 'Tingnan lahat' : 'View all'} <ChevronRight className="w-4 h-4" />
                </Link>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-yellow-500/20 to-orange-500/10 border border-yellow-500/30 rounded-2xl mb-4">
                ...
                <div>
                  <p className="text-yellow-400 font-black text-2xl">{gamification.totalPoints}</p>
                  <p className="text-xs text-slate-500 dark:text-gray-500">{accessibilitySettings.language === 'fil' ? 'Kabuuang Puntos' : 'Total Points'}</p>
                </div>
                <div className="ml-auto text-right">
                  <p className="text-slate-900 dark:text-white font-bold text-lg">{gamification.earned.length}</p>
                  <p className="text-xs text-slate-500 dark:text-gray-500">{accessibilitySettings.language === 'fil' ? 'Badge' : 'Badges'}</p>
                </div>
              </div>
              ...
              {gamification.earned.length > 0 ? (
                <div className="flex flex-wrap gap-3">
                  {gamification.earned.slice(0, 6).map((a: any, i: number) => (
                    <div key={i} title={a.title} className="flex flex-col items-center gap-1 p-3 bg-gradient-to-br from-yellow-900/20 to-orange-900/10 border border-yellow-500/20 rounded-2xl hover:border-yellow-400/40 transition-all cursor-default">
                      <span className="text-3xl">{a.icon}</span>
                      <span className="text-xs text-slate-600 dark:text-gray-400 max-w-[60px] text-center truncate">{a.title}</span>
                    </div>
                  ))}
                  {gamification.earned.length > 6 && (
                    <Link to="/achievements" className="flex flex-col items-center justify-center gap-1 p-3 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all w-[76px]">
                      <span className="text-slate-400 font-bold">+{gamification.earned.length - 6}</span>
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-slate-500 dark:text-gray-500 text-sm">{accessibilitySettings.language === 'fil' ? 'Kumpletuhin ang mga kurso para makakuha ng badge!' : 'Complete courses to earn badges!'}</p>
                </div>
              )}
            </div>
            ...
            <div className="backdrop-blur-2xl bg-white/5 border border-white/10 rounded-3xl p-8 shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold bg-gradient-to-r from-cyan-300 to-purple-300 bg-clip-text text-transparent flex items-center gap-2">
                  ...
                  {accessibilitySettings.language === 'fil' ? 'Leaderboard' : 'Leaderboard'}
                </h2>
                <Link to="/leaderboard" className="flex items-center gap-1 text-sm text-cyan-400 hover:text-cyan-300 transition-colors">
                  {accessibilitySettings.language === 'fil' ? 'Tingnan lahat' : 'View all'} <ChevronRight className="w-4 h-4" />
                </Link>
              </div>
              ...
              {gamification.topLeaderboard ? (
                <div className="p-5 bg-gradient-to-r from-cyan-500/20 to-purple-500/10 border border-cyan-500/30 rounded-2xl">
                  <p className="text-sm text-slate-600 dark:text-gray-400 mb-1">{gamification.topLeaderboard.name}</p>
                  <div className="flex items-center gap-3">
                    <div className="text-4xl font-black text-cyan-400">#{gamification.topLeaderboard.rank}</div>
                    <div>
                      <p className="text-slate-900 dark:text-white font-bold">{accessibilitySettings.language === 'fil' ? 'Iyong Ranggo' : 'Your Rank'}</p>
                      <p className="text-sm text-slate-500 dark:text-gray-500">{gamification.topLeaderboard.value} {accessibilitySettings.language === 'fil' ? 'puntos' : 'points'}</p>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="text-center py-6">
                  ...
                  <p className="text-slate-500 dark:text-gray-500 text-sm">{accessibilitySettings.language === 'fil' ? 'Kumpletuhin ang mga gawain para lumabas sa leaderboard!' : 'Complete activities to appear on the leaderboard!'}</p>
                </div>
              )}
              ...
              >
                ...
                {accessibilitySettings.language === 'fil' ? 'Tingnan ang Buong Leaderboard' : 'View Full Leaderboard'}
              </Link>
            </div>
          </div>
        </div>
